...
class DebateRooms(commands.Cog, name="Debate"):
    ...
    def check_in_debate(self, ctx: SlashContext):
    ...
    @cog_ext.cog_slash(
        name="topic",
        description="Set or vote for a topic in a debate room.",
        guild_ids = [768042322176770058],
        options=[
            {
                "name": "member",
                "description": "A member to get the topic from.",
                "type": 3,
                "required": False
            },
            {
                "name": "message",
                "description": "The message to set the topic to",
                "type": 4,
                "required": True
            }
        ]
    ...
    async def topic(
        self, ctx: SlashContext, member: Member, *, message: str
    ):
    ...
    @cog_ext.cog_slash(
        name="remove-topic",
        description="Remove a topic from a debate room.",
        guild_ids=[768042322176770058],
        options=[
            {
                "name": "member",
                "description": "A member to remove the topic from.",
                "type": 3,
                "required": False
            }
        ]
    )
    async def remove_topic(self, ctx: SlashContext, member: Member):
    ...
    @cog_ext.cog_slash(
        ...
        description="Permanently reset the ELO ratings of users.",
        guild_ids=[768042322176770058]
    )
    async def reset_scores(self, ctx: SlashContext):
        guild = ctx.guild
    ...
    @cog_ext.cog_slash(
        name="setup-elo",
        description="Initialize ELO ratings in a new server.",
        guild_ids=[768042322176770058]
    )
    async def setup_elo(self, ctx: SlashContext):
        guild = ctx.guild
    ...
    @cog_ext.cog_slash(
        name="repair-elo",
        description="Repairs the ELO ratings of a buggy user.",
        guild_ids=[768042322176770058],
        options=[
            {
                "name": "member",
                "description": "A member to repair the elo of",
                "type": 3,
                "required": True
            }
        ]
    )
    async def repair_elo(self, ctx: SlashContext, member: Member):
        if member.bot:
    ...
    @cog_ext.cog_slash(
        name="repair-all-elo",
        description="Repairs the ELO ratings of buggy users.",
        guild_ids=[768042322176770058]
    )
    async def repair_all_elo(self, ctx: SlashContext):
        guild = ctx.guild
    ...
    @cog_ext.cog_slash(
        name="elo",
        description="Display ELO rating of a user.",
        guild_ids=[768042322176770058],
        options=[
            {
                "name": "member",
    ...
    async def elo(self, ctx: SlashContext, member: Optional[discord.Member]):
        if member:
            if member.bot:
                return
        if member:
    ...
    @cog_ext.cog_slash(
        name="leaderboard",
        description="Display top 10 ELO ratings of a users.",
        guild_ids=[768042322176770058]
    )
    ...
    @cog_ext.cog_slash(
        name="for",
        description="Set the 'For' position on a topic in a debate room.",
        guild_ids=[768042322176770058]
    )
    async def command_for(self, ctx: SlashContext):
        room = self.get_room(self.get_room_number(ctx.channel))
    ...
    @cog_ext.cog_slash(
        name="against",
        description="Set the 'Against' position on a topic in a debate room.",
        guild_ids=[768042322176770058]
    )
    async def against(self, ctx: SlashContext):
        room = self.get_room(self.get_room_number(ctx.channel))
    ...
    @cog_ext.cog_slash(
        name="debate",
        description="Start or join a debate.",
        guild_ids=[768042322176770058]
    )
    async def debate(self, ctx: SlashContext):
        room = self.get_room(self.get_room_number(ctx.channel))
    ...
    @cog_ext.cog_slash(
        name="debate-for",
        aliases=["df"],
        description="Shortcut to debate for the topic.",
        guild_ids=[768042322176770058]
    )
    async def debate_for(self, ctx: SlashContext):
        room = self.get_room(self.get_room_number(ctx.channel))
    ...
    @cog_ext.cog_slash(
        name="debate-against",
        aliases=["da"],
        description="Shortcut to debate against the topic.",
        guild_ids=[768042322176770058]
    )
    async def debate_against(self, ctx: SlashContext):
        room = self.get_room(self.get_room_number(ctx.channel))
    ...
    @cog_ext.cog_slash(
        name="current-topic",
        aliases=["ct"],
        description="Display current topic of a given user.",
        guild_ids=[768042322176770058],
        options=[
            {
                "name": "member",
                "description": "A member to get the current topic from.",
                "type": 3,
                "required": True
            }
        ]
    )
    async def current_topic(self, ctx: SlashContext, author: discord.Member):
        room = self.get_room(self.get_room_number(ctx.channel))
    ...
    @cog_ext.cog_slash(
        name="vote",
        description="Vote for who you think won the debate.",
        guild_ids=[768042322176770058],
        options=[
            {
                "name": "member",
                "description": "A member to vote for",
                "type": 3,
                "required": True
            }
        ]
    )
    async def vote(self, ctx: SlashContext, member: discord.Member):
        room = self.get_room(self.get_room_number(ctx.channel))
    ...
    @cog_ext.cog_slash(
        name="private",
        description="Convert a public debate into a private one.",
        guild_ids=[768042322176770058]
    )
    async def private(self, ctx: SlashContext):
        room = self.get_room(self.get_room_number(ctx.channel))
        if not room.private:
            room.private = True
        else:
    ...
    @cog_ext.cog_slash(
        name="public",
        description="Convert a private debate back into a public debate.",
        guild_ids=[768042322176770058]
    )
    async def public(self, ctx: SlashContext):
        room = self.get_room(self.get_room_number(ctx.channel))
        if room.private:
            room.private = False
        else:
    ...
    @cog_ext.cog_slash(
        name="unlock",
        description="Unlocks a member in a private room to become a debater.",
        guild_ids=[768042322176770058],
        options=[
            {
                "name": "member",
                "description": "A member to unlock.",
                "type": 3,
                "required": False
            }
        ]
    )
    async def unlock(self, ctx: SlashContext, member: discord.Member):
        room = self.get_room(self.get_room_number(ctx.channel))
        if not room.private:
            embed = discord.Embed(
    ...
    @cog_ext.cog_slash(
        name="conclude",
        description="Conclude an active debate.",
        guild_ids=[768042322176770058]
    )
    async def conclude(self, ctx: SlashContext):
        room = self.get_room(self.get_room_number(ctx.channel))
        match = room.match
    ...
    @cog_ext.cog_slash(
        name="tutorial",
        description="Displays a youtube tutorial for the server.",
        guild_ids=[768042322176770058]
    )
    ...
    async def debates_disabled(self, ctx: SlashContext):
        self.exiting = True
