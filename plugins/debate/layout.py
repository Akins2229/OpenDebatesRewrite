...
class ServerSetup(commands.Cog, name="Server Setup"):
    ...
    @cog_ext.cog_slash(
        name="setup-roles",
        description="Set up roles required by the bot in the server.",
        guild_ids=[768042322176770058], # Open Debates Guild ID (if you want to use the bot place your own within the array as well)
    )
    async def setup_roles(self, ctx: SlashContext):
    ...
    async def check_roles(self, ctx: SlashContext, message):
        guild = ctx.guild
        title = f"❌ {message} ❌"
    ...
    async def check_channels(self, ctx: SlashContext, message):
        guild = ctx.guild
        title = f"❌ {message} ❌"
    ...
    @cog_ext.cog_slash(
        name="setup-channels",
        description="Set up channels required by the bot in the server.",
        guild_ids=[768042322176770058]
    )
    async def setup_channels(self, ctx: SlashContext):
        guild = ctx.guild
        self.role_everyone = guild.default_role
    ...
    @cog_ext.cog_slash(
        name="enable-debates",
        description="Enable the 'Debates' cog.",
        guild_ids=[768042322176770058]
    )
    async def enable_debates(self, ctx: SlashContext):
        # Indicate command is running.
    ...
    @cog_ext.cog_slash(
        ...
        description="Disable the 'Debates' cog.",
        guild_ids = [768042322176770058]
    )
    async def disable_debates(self, ctx: SlashContext):
        # Indicate command is running.
    ...
    @commands.command(
        name="lockdown",
        description="Locks the entire server down during downtime issues.",
        guild_ids=[768042322176770058]
    )
    ...
    async def lock_all_channels(self, ctx: SlashContext):
    ...
    @cog_ext.cog_slash(
        name="reopen",
        description="Unlocks the server for public use.",
        guild_ids = [768042322176770058]
    )
    async def reopen(self, ctx: SlashContext):
        # Indicate command is running.
    ...
    @commands.command(
        name="quit",
        description="Quit the bot and logout of bot account.",
        guild_ids=[768042322176770058]
    )
